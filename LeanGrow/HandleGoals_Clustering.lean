...
#eval goal_cluster_list.foldlM (fun _ t => IO.println (Trie.print_keys ⟨#[]⟩ t.name_list)) ()

#eval Trie.toList (goal_cluster_list.tail.foldl (fun M t => Trie.merge_count M (Trie.merge_count_initialise t.name_list)) (Trie.merge_count_initialise goal_cluster_list.head!.name_list))

#eval Trie.print_keys ⟨#[]⟩ (Trie.merge_count (Trie.merge_count_initialise goal_cluster_list.head!.name_list) (Trie.merge_count_initialise goal_cluster_list.tail.head!.name_list))
#eval Trie.toList (Trie.merge_count (Trie.merge_count_initialise goal_cluster_list.head!.name_list) (Trie.merge_count_initialise goal_cluster_list.tail.head!.name_list))


def process' (pd : gdata) (flag : Bool) (fact_cluster : ( (List gdata))) (count len : Nat) (ignore_core : Trie Unit): List (Trie Unit × (List gdata)) → (List (Trie Unit × (List gdata)) × ((List gdata)))
| [] =>
  --dbg_trace s!"Final step of process phase reached with flag {flag}"
  if flag then ([],[]) else (if ( Nat.toFloat (Trie.size pd.name_list)) == 0 then ([], pd :: fact_cluster) else ([(pd.name_list, [pd])], []))
| c :: cs =>
    --dbg_trace s!"Instepcting cluster with name list {Trie.print_keys ⟨#[]⟩ c.1}"
    let pd_shaded := Trie.filter pd.name_list ignore_core
    if count ≤ (len / 4)
    then
      let S := ( Nat.toFloat (Trie.size pd_shaded)) -- cache size in gdata!
      if S == 0
      then -- this whole part should be moved out of recursion!
        --dbg_trace s!"Empty name list → skipping"
        let (clu, facz) := (process' pd flag fact_cluster (count) len ignore_core cs)
        (c :: clu, facz)
      else
        let common := Trie.CountCommon pd_shaded c.1
        let candidate_ratio := ( Nat.toFloat common ) /  S
        let cluster_ratio := ( Nat.toFloat common ) / (Nat.toFloat (Trie.size (Trie.filter c.1 ignore_core)))
        --dbg_trace s!"Common names : {common} ; candidate ratio : {candidate_ratio} ; cluster ratio : {cluster_ratio}"
        -- v1
        -- if ratio ≥ 0.33
        -- then  let merged_trie := Trie.merge pd.name_list c.1
        ...
        then  --dbg_trace s!"Merging!"
              let merged_trie := Trie.merge pd.name_list c.1
              let clust := pd :: c.2
              let (clu, facz) := (process' pd true fact_cluster (count + 1) len ignore_core cs)
              ((merged_trie, clust) :: clu, facz)
        else --dbg_trace s!"Skipping"
             let (clu, facz) := (process' pd flag fact_cluster (count) len ignore_core cs)
             (c :: clu, facz)
      ...
      --dbg_trace s!"Insertion count above quater length → skipping"
      let (clu, facz) := (process' pd true fact_cluster (count) len ignore_core cs)
      (c :: clu, facz)

def cluster'  (clu : (List gdata)) : (List (Trie Unit × (List gdata))):=
  let merge_count_trie :=
    match clu with
    | [] => Trie.empty
    | _ => clu.tail.foldl (fun M t => Trie.merge_count M (Trie.merge_count_initialise t.name_list)) (Trie.merge_count_initialise clu.head!.name_list)
  let core_trie := Trie.cut merge_count_trie (Trie.size merge_count_trie) 0.2
  let rec c :  List gdata → (List (Trie Unit × (List gdata)) × ((List gdata)))
    | [] => ([], [])
    | pd :: rest =>
        let (sofar_c, sofar_f) := c rest
        --dbg_trace s!"\nEntering new process phase with {sofar_c.length}+1 clusters\nFitting {pd.cst_name} with data {Trie.print_keys ⟨#[]⟩ pd.name_list}\n"
        process' pd false sofar_f 0 sofar_c.length core_trie sofar_c
  let (petals, core) := c clu
  (core_trie, core) :: petals

elab "make_cluster_2" : command => do
  -- v1
  let res := cluster' goal_cluster_list
  -- v2
  -- dbg_trace s!"Clustering!"
  -- let (preres, eres) := cluster goal_cluster_list
  -- dbg_trace s!"Phase 2 !"
