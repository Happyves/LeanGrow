...
partial def propagate_solution
  (solveStatusB : List (Array Bool)) (sSB_handler : Nat → (Nat × Nat)) (sSB_paging : Nat)
  (solveStatusU : List (Array Bool)) (sSU_handler : Nat → (Nat × Nat)) (sSU_paging : Nat)
  (solveStatusG : List (Array SolveType)) (sSG_handler : Nat → (Nat × Nat)) (sSG_paging : Nat)
  (backStep_dict : List (Nat × BackStepData))
  (uniStep_dict : List (Nat × UniData))
  (goal_dict : List (Nat × GoalData))
  (uniStepId : Nat) :
  (List (Array Bool)) × (List (Array Bool)) × (List (Array SolveType)) :=
  let rec main_uni (uni : Nat) (sSB : List (Array Bool)) (sSU : List (Array Bool)) (sSG : List (Array (Option Nat))):
    (List Nat) × (List (Array Bool)) × (List (Array Bool)) × (List (Array (Option Nat))) :=
    match uniStep_dict.find? (fun x => x.1 == uni) with
    | .none => ([],[],[],[])
    | .some (_, data) =>
        let proceed? := data.subgoalSubst.foldl
          (fun B g =>
            if B
            then
              let (sp,si) := sSG_handler g.2.goalId
        ...
        if proceed?
        then
          let nsSU := PageingSet sSU sSU_handler sSU_paging false uni true
          let nsSG := PageingSet sSG sSG_handler sSG_paging .none data.targetId (.ofUni uni)

#exit

          ((.ofG data.targetId) :: (data.assigns.map (fun x => .ofB x.1 x.2.1)), sSB, nsSU, sSG)
        else
          ([], sSB, sSU, sSG)
  let rec main_back (back : Nat) (sSB : List (Array Bool)) (sSU : List (Array Bool)) (sSG : List (Array (Option Nat))):
    (List Helper) × (List (Array Bool)) × (List (Array Bool)) × (List (Array (Option Nat))) :=
    ...
    | .some (_, data) =>
        ...
        then
          ...
          ([.ofG data.targetId], nsSB, sSU, sSG)
        else
          ([], sSB, sSU, sSG)
  let rec main_goal (of : Helper) (sSB : List (Array Bool)) (sSU : List (Array Bool)) (sSG : List (Array (Option Nat))):
    (Nat) × (List (Array Bool)) × (List (Array Bool)) × (List (Array (Option Nat))) :=
    match goal_dict.find? (fun x => x.1 == goal) with
    | .none => (0,[],[],[])
    | .some (_, data) =>
