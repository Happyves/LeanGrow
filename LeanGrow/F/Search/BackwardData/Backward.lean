...
def thmAppli_from_solve (goalIdCounter backStepIdCounter : Nat)
  (thm_data : Array EmbedData) : Unit := sorry
/- should be consequence of solving a goal that causes propagation:
we consider the thm-application/backstep that this subgoal was generated by,
and make a copy of it, which should however get a new id and be declare to solve
the same goal as the original, where we fill the `ltxSols` list with the solution
