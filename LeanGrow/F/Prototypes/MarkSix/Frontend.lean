...
import LeanGrow.F.Prototypes.MarkSix.Search
...
def init_FixCtx (env : Environment) (init_gnodeTypes : Array CExpr) : FixCtx :=
  ...
  let T := env.constants.map₁.fold
    (fun sofar name info =>
      if (Name.isPrefixOf `Init modules[env.const2ModIdx[name].get! (α := Nat)]!) || (Name.isPrefixOf `LeanGrow.F.Prototypes.MarkSix.TestTypes modules[env.const2ModIdx[name].get! (α := Nat)]!)
...
elab "grow" : tactic => do
  let ref ← getRef
  let dEnv ← simpleImportModules #[`LeanGrow.F.Prototypes.MarkSix.TestTypes]
