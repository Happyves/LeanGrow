...
def List.rangeOne (n : Nat) : List Nat :=
  let rec go (out : List Nat) : Nat → List Nat
    | 0 => out
    | n+1 => go ((n+1) :: out) n
  go [] n

#eval List.rangeOne 3
...
def integrate_backstep (fctx : FixCtx) (f_id_gen : Nat)
  ...
  (backTree : BackTree) : Nat × List (Nat × CExpr) × IntegBack :=
    ...
    let new_leaves_1 : Array BackTree := (Array.mkArray thm_data_size .fail)
    ...
    let (new_leaves_3,nfid,ngns,commonFix,_) := newgoals.foldl
      (fun (A,fid,ngns,cF,ngi) (pos, type) =>
          match intro? type fid with
          | .none =>
              let vers := (cexprReduceShallow fctx 3 type).eraseDup
              let (red, nngi) := vers.foldl (fun (L,i) x => ((pos,i,x) :: L, i+1)) ([],ngi+1)
              (A.set! pos (.ofGoal ngi type []
                (red.map (fun x => x.2.1))
                (red.map (fun (_,id,ty) => .ofGoal id ty [] [] []))),
                  fid,ngns,(pos, ngi, type):: (red ++ cF), nngi)
          | .some (nfig,ng,ngnodes) =>
              let inter_ngnode := ngnodes ++ ngns
              let newforw2 := inter_ngnode.foldl
                  (fun sofar (idx,exp) => PageingSet sofar fctx.gnodeTypesHandler 42 (.failed) idx exp)
                  fctx.gnodeTypes
              let fix_fctx := {fctx with gnodeTypes := newforw2}
              let vers := (cexprReduceShallow fix_fctx 3 type).eraseDup
              let (red, nngi) := vers.foldl (fun (L,i) x => ((pos,i,x) :: L, i+1)) ([],ngi+1)
              (A.set! pos (.ofIntro ngi ng ngnodes []
                (red.map (fun x => x.2.1))
                (red.map (fun (_,id,ty) => .ofGoal id ty [] [] []))),
                  nfig,inter_ngnode, (pos, ngi, ng):: (red ++ cF), nngi)
          )
      (new_leaves_2,f_id_gen,[],[],id_gen_goal)
    let new_dirs := commonFix.map (fun x => x.2.1)
    let dirs_new := commonFix.foldl
      (fun A (pos,gId,_) => A.set! pos [gId])
      (Array.mkArray thm_data_size [])
...
def integrate_backstep_main (fctx : FixCtx)  (f_id_gen : Nat)
  (thm_name : BackType) (thm_data_size : Nat) (target_goal_id : Nat)
  (assigned newgoals : List (Nat × CExpr))
  (state : BackState) : Nat × List (Nat × CExpr) × BackState :=
  let (nfid,ngnds,⟨G,T⟩) := integrate_backstep fctx f_id_gen thm_name thm_data_size target_goal_id assigned newgoals state.id_gen_back state.id_gen_goal state.bt
...
partial def propagate_uni_assign_toGoalsAssigns (fctx : FixCtx) (f_id_gen : Nat)
    ...
    | .ofGoal gid type gbd ggd sols =>
        ...
        then -- no point in trying intro, as we try it at the originating backstep, and nothing changed
          ...
          | .none =>
              ...
              let vers := (cexprReduceShallow fctx 3 propad?).eraseDup
              ...
              let redG := (.ofPropa uni_id (ngi) propad? gbd
                ...
                ((red.map (fun (id,ty) => .ofGoal id ty [] [] [])) ++  fsols))
              let propa! : BackTree := .ofGoal gid type gbd (((List.range ((nngi + 1) - id_gen_goal)).map (· + id_gen_goal)) ++ ggd) (redG :: fsols)
              --dbg_trace s!"(propagate_uni_assign_toGoalsAssigns) local return {repr (propa!,(ngi,propad?) :: ngs, ngi+1)}"
              (new_f_id_gen, new_gnode,propa!,(ngi,propad?) :: (red ++ ngs), nngi+1)
          | .some (ff_id_gen, f_goal, add_gnodes) =>
              ...
              let inter_ngnode := add_gnodes ++ (new_gnode.map Prod.snd).join
              let newforw2 := inter_ngnode.foldl
                  (fun sofar (idx,exp) => PageingSet sofar fctx.gnodeTypesHandler 42 (.failed) idx exp)
                  fctx.gnodeTypes
              let fix_fctx := {fctx with gnodeTypes := newforw2}
              let vers := (cexprReduceShallow fix_fctx 3 propad?).eraseDup
              let (red, nngi) := vers.foldl (fun (L,i) x => ((i,x) :: L, i+1)) ([],ngi+1)
              let redG := (.ofPropa uni_id (nngi+1) propad? gbd
                (((List.range ((nngi+1) - id_gen_goal)).map (· + id_gen_goal)) ++ ggd)
                ((.ofIntro nngi f_goal add_gnodes
                    [] (red.map Prod.fst)
                    (red.map (fun (id,ty) => .ofGoal id ty [] [] []))) ::  fsols))
              let propa! : BackTree := .ofGoal gid type gbd (((List.range ((nngi + 2) - id_gen_goal)).map (· + id_gen_goal)) ++ ggd) (redG :: fsols)
              --dbg_trace s!"(propagate_uni_assign_toGoalsAssigns) local return {repr (propa!,(ngi,propad?) :: ngs, ngi+1)}"
              (ff_id_gen, (nngi, add_gnodes) :: new_gnode,propa!,(nngi+1,propad?) :: (nngi, f_goal):: (red ++ ngs), nngi+2)
        ...
        then
          ...
          | .none =>
              ...
              let redG := (.ofPropa uni_id (ngi) propad? gbd
                (((List.range ((nngi) - id_gen_goal)).map (· + id_gen_goal)) ++ ggd)
                ((red.map (fun (id,ty) => .ofGoal id ty [] [] [])) ++  fsols))
              let propa! : BackTree := .ofPropa uid gid type gbd (((List.range ((nngi + 1) - id_gen_goal)).map (· + id_gen_goal)) ++ ggd) (redG :: fsols)
              --dbg_trace s!"(propagate_uni_assign_toGoalsAssigns) local return {repr (propa!,(ngi,propad?) :: ngs, ngi+1)}"
              (new_f_id_gen, new_gnode,propa!,(ngi,propad?) :: (red ++ ngs), nngi+1)
          | .some (ff_id_gen, f_goal, add_gnodes) =>
              --dbg_trace s!"(propagate_uni_assign_toGoalsAssigns) intro? returned {repr (ff_id_gen, f_goal, add_gnodes)}"
              ...
              let redG := (.ofPropa uni_id (nngi+1) propad? gbd
                ...
                ((.ofIntro nngi f_goal add_gnodes
                    [] (red.map Prod.fst)
                    (red.map (fun (id,ty) => .ofGoal id ty [] [] []))) ::  fsols))
              let propa! : BackTree := .ofPropa uid gid type gbd (((List.range ((nngi + 2) - id_gen_goal)).map (· + id_gen_goal)) ++ ggd) (redG :: fsols)
              --dbg_trace s!"(propagate_uni_assign_toGoalsAssigns) local return {repr (propa!,(ngi,propad?) :: ngs, ngi+1)}"
              (ff_id_gen, (nngi, add_gnodes) :: new_gnode,propa!,(nngi+1,propad?) :: (nngi, f_goal):: (red ++ ngs), nngi+2)
        ...
        then
          ...
          | .none =>
              ...
              let redG := (.ofPropa uni_id (ngi) propad? gbd
                (((List.range ((nngi) - id_gen_goal)).map (· + id_gen_goal)) ++ ggd)
                ((red.map (fun (id,ty) => .ofGoal id ty [] [] [])) ++  fsols))
              let propa! : BackTree := .ofIntro gid type bvs gbd (((List.range ((nngi + 1) - id_gen_goal)).map (· + id_gen_goal)) ++ ggd) (redG :: fsols)
              --dbg_trace s!"(propagate_uni_assign_toGoalsAssigns) local return {repr (propa!,(ngi,propad?) :: ngs, ngi+1)}"
              (new_f_id_gen, new_gnode,propa!,(ngi,propad?) :: (red ++ ngs), nngi+1)
          | .some (ff_id_gen, f_goal, add_gnodes) =>
              --dbg_trace s!"(propagate_uni_assign_toGoalsAssigns) intro? returned {repr (ff_id_gen, f_goal, add_gnodes)}"
              ...
              let redG := (.ofPropa uni_id (nngi+1) propad? gbd
                ...
                ((.ofIntro nngi f_goal add_gnodes
                    [] (red.map Prod.fst)
                    (red.map (fun (id,ty) => .ofGoal id ty [] [] []))) ::  fsols))
              let propa! : BackTree := .ofIntro gid type bvs gbd (((List.range ((nngi + 2) - id_gen_goal)).map (· + id_gen_goal)) ++ ggd) (redG :: fsols)
              --dbg_trace s!"(propagate_uni_assign_toGoalsAssigns) local return {repr (propa!,(ngi,propad?) :: ngs, ngi+1)}"
              (ff_id_gen, (nngi, add_gnodes) :: new_gnode,propa!,(nngi+1,propad?) :: (nngi, f_goal):: (red ++ ngs), nngi+2)
    | x =>
...
def integrate_uni? (f_id_gen : Nat)
  ...
  (init_tree : BackTree) : Option (List (Nat × Nat × CExpr) × Nat × List (Nat × List (Nat × CExpr)) × BackTree × List (Nat × CExpr) × Nat) :=
    ...
    | .some (TA,A) =>
        ...
        (A, propagate_uni_assign_toGoalsAssigns fctx f_id_gen uni_id paramNames lvls A id_gen_goal TA)
    | .none => .none
