...
def embed_next_raw (ltx : List (Nat × CExpr))
  ...
  let res := ((lTrace TraceFlags.zero & s!"Intermediate candidates in embed_next_raw: {repr candidates}" & candidates).map
      (fun (e, tp) => merge_if_compatible (embedSofar.set! todo_idx (.some (.gnode e (.ofBvar 42)))) tp)).reduceOption
...
def propagate_raw (fctx : FixCtx)
  (embedSofar :  Array (Option CExpr)) (todo_idx : Nat) (todo_thm_type : CExpr) : Option ((Array (Option CExpr)) × (List Nat)) :=
  let res :=
    match embedSofar.get! todo_idx with
    | .none => .none
    | .some ce =>
          let cet := CExpr.inferType fctx ce
          match CExpr.MatchAssignLFFC todo_thm_type cet with
          | .none => .none
...
partial def full_matcher_raw (fctx : FixCtx)
  ...
  let rec main (embedSofar :  Array (Option CExpr)) (instances : List Nat) (unassignedNodes : List Nat) (assignedFrontier : List Nat) : List EmbedStruct :=
    ...
    | n :: l =>
        ...
        match propagate_raw fctx embedSofar n nd.cexpr with
