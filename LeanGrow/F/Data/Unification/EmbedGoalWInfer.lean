...
partial def match_goal (fctx : FixCtx)
  ...
  (thm_hyp_num : Nat) (thm_goal real_goal : CExpr) : Option (Array (Option CExpr)) :=
    let rec gop (embed : Array (Option CExpr)) : List Nat → Option (Array (Option CExpr))
      | [] => .some embed
      | n :: l =>
            let nd := thm_data.get! n
            match propagate_goal fctx embed n nd.cexpr with
            | .none => .none
            | .some (emb, toFront) => gop emb (toFront ++ l)
    with_lTrace [TraceFlags.zero] in
    match CExpr.MatchAssignLFFC thm_goal real_goal with -- real_goal is a cexpr wrt. ltx
    | .none => .none
    | .some l =>
