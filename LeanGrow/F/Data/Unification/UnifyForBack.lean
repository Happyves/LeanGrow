...
partial def CExpr.MatchAssignSolutions' (s g : CExpr) : Option (List (Nat × Nat × CExpr)) :=
      let rec go (go? : Bool) (Aout : Option (List (Nat × Nat × CExpr))) (todo : List (CExpr × CExpr)) : Option (List (Nat × Nat × CExpr)) :=
      if go?
      then
            match todo with
            | [] => Aout
            | nx :: L =>
                  match nx with
                  | (.gnode i o, .lnode j _ (.some t)) => let Aup := List.assignOrFail' Aout t j (.gnode i o) ; go true Aup L
                  | (e, .lnode j _ (.some t)) => let Aup := List.assignOrFail' Aout t j (e) ; go true Aup L
                  | (.gnode i _, .gnode j _) => go (i == j) Aout L
                  | (.bvar i , .bvar j) => go (i == j) Aout L
                  | (.sort _, .sort _) => go true Aout L
                  | (.const n _, .const n' _) =>  go (n == n')  Aout L
