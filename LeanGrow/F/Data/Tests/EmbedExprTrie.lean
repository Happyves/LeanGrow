...
def test_1 (n : Name) : CoreM Unit := do
  ...
  -- let embeds := full_matcher_smoothF thmdata order fake_ltx_cexpr_idx fake_ltx_idx_cexpr fake_ltx_handler
  let embeds := partial_matcher_smoothF thmdata order fake_ltx_cexpr_idx fake_ltx_idx_cexpr fake_ltx_handler
