...
def cInferProjType (fctx : FixCtx) (bvarCtx : List CExpr)
  (structName : Name) (idx : Nat) (e : CExpr)
  (cexprInferType  cexprWhnf : FixCtx → List CExpr→ CExpr → CExpr) : CExpr :=
  ...
  | .const _ lvls =>
      ...
      | .some (.struc _ _ ctor _) =>
            let ctorType := cInferAppType fctx bvarCtx (.const ctor lvls) ps cexprInferType  cexprWhnf
...
def cInferLambdaType (fctx : FixCtx) (bvarCtx : List CExpr)
  (e : CExpr)
  (cexprInferType : FixCtx → List CExpr→ CExpr → CExpr) : CExpr :=
...
def getLevel (fctx : FixCtx) (bvarCtx : List CExpr)
  (type : CExpr)
  (cexprInferType  cexprWhnf : FixCtx → List CExpr→ CExpr → CExpr) : Level :=
  let typeType :=
    cexprWhnf fctx bvarCtx
...
def cInferForallType (fctx : FixCtx) (bvarCtx : List CExpr)
  ...
  let rec go (prepend : List CExpr) (sofar : List Level) : CExpr → List Level
    | .forallE _ t b _ => go (t :: prepend) ((getLevel fctx prepend t cexprInferType  cexprWhnf ) :: sofar) b
    | e => (getLevel fctx prepend e cexprInferType  cexprWhnf ) :: sofar
...
def cexprInferTypeImp
  ...
  | .proj n i s    => cInferProjType fctx bvarCtx n i s cexprInferType  cexprWhnf
  | .app ..      => let (h,as) := CExpr.getApp e ; cInferAppType fctx bvarCtx h as cexprInferType  cexprWhnf
  | .bvar bidx     =>
  ...
  | .forallE ..    => cInferForallType fctx bvarCtx e cexprInferType  cexprWhnf
  | .lam ..        => cInferLambdaType fctx bvarCtx e cexprInferType
  | .letE ..       => cInferLambdaType fctx bvarCtx e cexprInferType
  | .gnode gidx _ =>
