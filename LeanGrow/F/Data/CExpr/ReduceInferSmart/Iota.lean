...
import LeanGrow.F.Data.CExpr.ReduceInferSmart.FixCtx
import LeanGrow.F.Data.CExpr.ReduceInferSmart.BetaZeta
import Lean
...
def myForallBoundedTelescope (fctx : FixCtx) (bvarCtx : List CExpr)
  (auxAppType : CExpr)
  (cexprWhnf : FixCtx → List CExpr→ CExpr → CExpr)
  : Nat → CExpr × List CExpr
  | 0 => (auxAppType, bvarCtx)
  | n+1 =>
    match auxAppType with
    | .forallE _ t b _ => myForallBoundedTelescope fctx (t :: bvarCtx) b cexprWhnf n
    | _ =>
...
def CExpr.reduceMatcher? (fctx : FixCtx) (bvarCtx : List CExpr)
  (e : CExpr) (cexprInferType  cexprWhnf : FixCtx → List CExpr→ CExpr → CExpr)
  : CExpr_ReduceMatcherResult :=
  ...
  | .const n lvl =>
      ...
      | .some (.mat paras _ val info) =>
            ...
            then
              ...
              let (_,BV) := myForallBoundedTelescope fctx bvarCtx auxAppType cexprWhnf info.numAlts
...
def CExpr.toCtorWhenK (fctx : FixCtx) (bvarCtx : List CExpr)
  (recName : Name) (recVal : cRecursorVal) (major : CExpr)
...
def CExpr.reduceRec (fctx : FixCtx) (bvarCtx : List CExpr)
  (recName : Name) (recLvlParams : List Name) (recVal : cRecursorVal) (recLvls : List Level) (recArgs : List CExpr) (on : CExpr)
  (cexprInferType  cexprWhnf : FixCtx → List CExpr→ CExpr → CExpr) : CExpr :=
  let majorIdx := recVal.getMajorIdx
  if H : majorIdx < recArgs.length
  then
    let major := cexprWhnf fctx bvarCtx (recArgs.get ⟨majorIdx, H⟩)
    let eta_1 := if recVal.k then (CExpr.toCtorWhenK fctx bvarCtx recName recVal major cexprInferType  cexprWhnf) else major
    let eta_2 := eta_1.toCtorIfLit
    let eta_3 := CExpr.toCtorWhenStructure fctx bvarCtx
      recName.getPrefix eta_2 cexprInferType  cexprWhnf
...
def CExpr.reduceQuotRec (fctx : FixCtx) (bvarCtx : List CExpr)
  (recVal : QuotVal) (recArgs : List CExpr) (on : CExpr)
  (cexprWhnf : FixCtx → List CExpr→ CExpr → CExpr) : CExpr :=
