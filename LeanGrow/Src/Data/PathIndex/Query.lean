...
partial def queryCoreWW [Repr IdxCollType]
    ...
    let rec @[specialize] go (l1 : LocalContext) (l2 : LocalInstances)
      ...
      (ankers : ListProd3S IdxCollType (ListProd3SigL Expr (PaIn IdxCollType) (List FVarId) α) α)
      (todo : ListProd3SigL Expr (PaIn IdxCollType) (List FVarId) α) : MetaM (Prod5 UInt8 IdxCollType α LocalContext LocalInstances) := do
      mtracing
      if revert?
      then
        ...
        then return ⟨0,constr,state,l1,l2⟩ --fail state
          ...
          | .cons e T workas moreT =>
              ...
              | .br fvars mvars bvars sorts consts lits apf apa api laf laa lai alf ala ali lef lea lez lei projs proofsOf proofs =>
                  let ⟨constr,state,l1,l2⟩ ← mProcess e workas mvars constr state l1 l2
                    ...
                    | .fvar fid =>
                        ...
                        | .num (.str _ kind) i =>
                          ...
                          then
                            ...
                            let ⟨fstarg,constr,state,l1,l2⟩ ← fProcess fid fvars constr state
                              naive? l1 l2
                            if fstarg == 0
                            then
                              match ← fid.GetDecl l1 l2 with
                              | .cdecl .. => return ⟨2,constr,state,l1,l2⟩ -- naiveFail
                              | .ldecl _ _ _ _ V .. => go l1 l2 constr state naive? throw? revert? skipP revCount ankers (.cons V T workas moreT)
                            else if fstarg == 1 -- failure in non-naive case : revert
                            then
                              match ← fid.GetDecl l1 l2 with
                              | .cdecl .. => go l1 l2 constr state naive? throw? true skipP revCount ankers todo
                              | .ldecl _ _ _ _ V .. => go l1 l2 constr state naive? throw? revert? skipP revCount ankers (.cons V T workas moreT)
                            else -- success, set `throw?` to true
                              go l1 l2 constr state false true revert? skipP revCount ankers moreT
                        | _ =>
                          ...
                          then
                            ...
                            go l1 l2 constr state false true revert? skipP revCount ankers moreT
                    | .mvar fid =>
                        let ⟨fstarg,constr,state,l1,l2⟩ ← mvProcess fid mvars workas T constr state
                          naive? l1 l2
                        if fstarg == 0
                        then return ⟨2,constr,state,l1,l2⟩ -- naiveFail
                        else if fstarg == 1 -- failure in non-naive case : revert
      ...
      then
        ...
        then return ⟨0,constr,state,l1,l2⟩ --fail state
          ...
          | .cons e T workas moreT =>
              ...
              | .dead => return ⟨1,constr,state,l1,l2⟩ --deadC constr state
              | .br fvars mvars bvars sorts consts lits apf apa api laf laa lai alf ala ali lef lea lez lei projs proofsOf proofs =>
                  let ⟨savef,saveI,saveS,l1,l2⟩ ← mProcess e workas mvars constr state l1 l2
