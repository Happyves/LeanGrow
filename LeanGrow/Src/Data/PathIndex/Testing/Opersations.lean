...
def testMerge : Array Expr → Array Expr → MetaM Unit
  | Gnodes, Objs => do
      let mut L := []
      let mut R := []
      let mut i := 0
      ...
      for O in Objs do
        ...
        then
          ...
          R := (i,O) :: R
        i := i+1
      let ⟨Tl,l1,l2⟩ ← PaIn.ofList (← getLCtx) (← getLocalInstances) L .dead UInt32Array.empty (fun x => UInt32Array.single x.toUInt32)
...
With context (n : Nat) (m : Nat) and objects (n+m), (n-m), (n*m), (m+n), (fun x : Nat => ∀ y : Nat, x = y), (fun x : Nat => ∀ y : Nat, x = y+1) run testMerge

#check 1

With context (n : Nat) (x : Fin n) (y : Nat × Nat) and objects (y.1 = 42), (x.val = x.val + y.2), (1=1), (y.2 = 42), (x.val = x.val + 37), (1=1) run testMerge

#check 2

def testMapInds : Array Expr → Array Expr → MetaM Unit
  | Gnodes, Objs => do
      let mut L := []
      let mut i := 0
      for O in Objs do
...
With context (n : Nat) (m : Nat) and objects (n+m), (n-m), (n*m), (m+n), (fun x : Nat => ∀ y : Nat, x = y), (fun x : Nat => ∀ y : Nat, x = y+1) run testMapInds

def testDelete : Array Expr → Array Expr → MetaM Unit
  | Gnodes, Objs => do
      let mut L := []
      let mut i := 0
      for O in Objs do
...
With context (n : Nat) (m : Nat) and objects (n+m), (n-m), (n*m), (m+n), (fun x : Nat => ∀ y : Nat, x = y), (fun x : Nat => ∀ y : Nat, x = y+1) run testDelete
