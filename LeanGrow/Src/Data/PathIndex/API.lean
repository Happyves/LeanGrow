...
@[specialize]
def ListProd.insert_pain_core_multi
...
@[specialize]
def ListProd.insert_pain_nat :=
  @ListProd.insert_pain_core Nat (fun x y => x ≤ y) IdxCollType inferInstance

@[specialize]
def ListProd.insert_pain_nat_multi :=
  @ListProd.insert_pain_core_multi Nat (fun x y => x ≤ y) IdxCollType inferInstance


def ListProd.find_pain_nat :=
  @ListProd.find_pain_core Nat (fun x y => x ≤ y) IdxCollType inferInstance


@[specialize]
def ListProd.insert_pain_lex :=
  @ListProd.insert_pain_core (Nat × Nat) (fun x y =>
...
@[specialize]
def ListProd.insert_pain_lex_multi :=
  @ListProd.insert_pain_core_multi (Nat × Nat) (fun x y =>
    match compare x.1 y.1 with
    | .lt => true
    | .eq => x.2 ≤ y.2
    | .gt => false
...
def ListProd.find_pain_lex :=
  @ListProd.find_pain_core (Nat × Nat) (fun x y =>
    match compare x.1 y.1 with
    | .lt => true
    | .eq => x.2 ≤ y.2
    | .gt => false
...
@[specialize]
def ListProd.insert_pain_const
  (singleton : Nat → IdxCollType) (insert : Nat → IdxCollType → IdxCollType)
...
@[specialize]
def ListProd.insert_pain_const_multi
  (insert : IdxCollType → IdxCollType → IdxCollType)
...
@[specialize]
def ListProd.insert_pain_var
  (singleton : Nat → IdxCollType) (insert : Nat → IdxCollType → IdxCollType)
  (L : CTrie (IdxCollType)) (new : Name) (idx : Nat)
  : CTrie (IdxCollType) :=
  L.upsert new.toString.toUTF8 (fun
    | .none => .some <| (singleton idx)
    | .some E => .some <|  insert idx E
    )

@[specialize]
def ListProd.insert_pain_var_multi
  (insert : IdxCollType → IdxCollType → IdxCollType)
  (L : CTrie IdxCollType) (newN : Name) (idx : IdxCollType)
  : CTrie IdxCollType :=
  L.upsert newN.toString.toUTF8 (fun
    | .none => .some <| idx
    | .some E => .some <| insert idx E
...
@[specialize]
def ListProd.insert_pain_proj
  (l1 : LocalContext) (l2 : LocalInstances)
  (singleton : Nat → IdxCollType) (insert : Nat → IdxCollType → IdxCollType)
  (L : CTrie (ListProd IdxCollType (Nat × PaIn IdxCollType)))
  (skipP : Bool) (newN : Name) (newI : Nat) (newE : Expr) (idx : Nat) (depth : Nat)
  (pinsert : Bool → Nat → Expr → Nat → PaIn IdxCollType → LocalContext → LocalInstances → MetaM (Prod3 (PaIn IdxCollType) LocalContext  LocalInstances))
  : MetaM (Prod3 (CTrie (ListProd IdxCollType (Nat × PaIn IdxCollType))) LocalContext  LocalInstances) :=
    L.upsertL l1 l2 newN.toString.toUTF8 (fun
      ...
      | .some E, l1, l2 => do
          let ⟨res,l1,l2⟩  ← @ListProd.modifyAddSpe l1 l2 IdxCollType (Nat × PaIn IdxCollType)
            ⟨fun x y => x.1 == y.1⟩ E
            (fun x y l1 l2 => do
...
@[specialize]
def ListProd.insert_pain_proj_multi
  (l1 : LocalContext) (l2 : LocalInstances)
  (insert : IdxCollType → IdxCollType → IdxCollType)
  (L : CTrie (ListProd IdxCollType (Nat × PaIn IdxCollType)))
  (skipP : Bool) (newN : Name) (newI : Nat) (newE : Expr) (idx : IdxCollType) (depth : Nat)
  (pinsert : Bool → Nat → Expr → IdxCollType → PaIn IdxCollType → LocalContext → LocalInstances → MetaM (Prod3 (PaIn IdxCollType) LocalContext  LocalInstances))
  : MetaM (Prod3 (CTrie (ListProd IdxCollType (Nat × PaIn IdxCollType))) LocalContext  LocalInstances) :=
    L.upsertL l1 l2 newN.toString.toUTF8 (fun
      | .none, l1, l2 => do
          let ⟨res,l1,l2⟩ ← pinsert skipP depth newE idx .dead l1 l2
