...
def findRevert
  ...
  : Expr → PaInG IdxCollType → List FVarId → IdxCollType → Unit → LocalContext → LocalInstances → MetaM (Prod5 Bool IdxCollType Unit LocalContext LocalInstances) :=
...
partial def findCore [Repr IdxCollType]
    ...
    : MetaM (Prod5 UInt8 IdxCollType Unit LocalContext LocalInstances) :=
      ...
      queryCore l1 l2
        ...
        (fun n l bvs _ _ constr uni naive? l1 l2 =>
          match ListProd.find_PaInG_lex bvs (n, l) with
          | .none => if naive? then return ⟨0,constr,uni,l1,l2⟩ else return ⟨1,constr,uni,l1,l2⟩
          | .some inds _ =>
              let constr := intersect inds constr
              if empty? constr
        ...
        (fun n i j bvs _ _ constr uni naive? l1 l2 =>
          match ListProd.find_PaInG_lnode bvs n (i,j) with
          | .none => if naive? then return ⟨0,constr,uni,l1,l2⟩ else return ⟨1,constr,uni,l1,l2⟩
          | .some inds _ =>
              let constr := intersect inds constr
              if empty? constr
