...
partial def BackTree.propagateForBackAssembly
  (l1 : LocalContext) (l2 : LocalInstances)
  (id_gen_goal : Nat) (unif_claches : ListProd Nat (UInt32Array)) (bt : BackTree)
  (propaGs: ListProd Nat Expr) (goalSpawn : Array (OptionProd Nat Nat))
  (ta : ListProd3 Nat Nat Expr) (unif_id : UInt32Array)
  ...
  let goalPropaCase (id_gen_goal : Nat) (propaGs: ListProd Nat Expr) (goalSpawn : Array (OptionProd Nat Nat))
    (go : Nat → ListProd Nat Expr → Array (OptionProd Nat Nat) →  BackTree → MetaM (Prod5 BackTree Nat (ListProd Nat Expr) (UInt32Array) (Array (OptionProd Nat Nat) )))
    (pass goal_id : Nat) (type : Expr) (bdirs gdirs : UInt32Array) (args : List BackTree)
    (x : BackTree) (prefixB : Nat → Nat → Expr → UInt32Array → UInt32Array → List BackTree → BackTree) :
    MetaM (Prod5 BackTree Nat (ListProd Nat Expr) (UInt32Array)  (Array (OptionProd Nat Nat))) := do
      ...
      then
        ...
        return ⟨x, id_gen_goal, propaGs, .empty,goalSpawn⟩
      else
        ...
        let ⟨args,id_gen_goal,propaGs,nGs,goalSpawn⟩ ← args.foldlM (fun ⟨args,id_gen_goal,propaGs,nGs,goalSpawn⟩ bt => do
          ...
          return ⟨a :: args, id_gen_goal,propaGs, .union nnGs nGs,goalSpawn⟩) (⟨[],id_gen_goal,propaGs,.empty,goalSpawn⟩ : Prod5 (List BackTree) Nat (ListProd Nat Expr) (UInt32Array) (Array (OptionProd Nat Nat)))
        let newBr := BackTree.ofPropa pass unif_id id_gen_goal newtype .empty .empty []
        ...
        if redtype == newtype
        then
          let res := prefixB pass goal_id type bdirs (gdirs.push id_gen_goal.toUInt32) (newBr :: args)
          return ⟨res, id_gen_goal + 1, .cons (id_gen_goal) newtype propaGs, nGs.push id_gen_goal.toUInt32, goalSpawn.push (goalSpawn[id_gen_goal]!)⟩
        else
          let newBr2 := BackTree.ofPropa pass unif_id (id_gen_goal + 1) redtype .empty .empty []
          let res := prefixB pass goal_id type bdirs ((gdirs.push id_gen_goal.toUInt32).push (id_gen_goal + 1).toUInt32) (newBr :: newBr2 :: args)
          return ⟨res, (id_gen_goal + 2), .cons (id_gen_goal + 1) redtype (.cons (id_gen_goal) newtype propaGs), ((nGs.push id_gen_goal.toUInt32).push (id_gen_goal + 1).toUInt32), goalSpawn.pushN (goalSpawn[id_gen_goal]!) 2⟩
  let rec go (id_gen_goal : Nat) (propaGs: ListProd Nat Expr) (goalSpawn : Array (OptionProd Nat Nat))
    : BackTree → MetaM (Prod5 BackTree Nat (ListProd Nat Expr) (UInt32Array) (Array (OptionProd Nat Nat)))
    | x@(.fail ..) | x@(.ofUni ..) => return ⟨x,id_gen_goal,propaGs,.empty,goalSpawn⟩
    ...
    | (.ofBack pass back_id mdata thm bdirs gdirs args) => do
        ...
        for i in [:args.size] do
          ...
          mtrace on .zero with s!"[propagateForBackAssembly] nnGs : {nnGs}"
          args := args.set! i nbt
          gdirs := gdirs.modify i (fun ds => .union ds nnGs)
          nGs := UInt32Array.union nGs nnGs
          propaGs := npropaGs
          id_gen_goal := nst
          gS := ngoalSpawn
    ...
    | (.ofIntro pass back_id gnIdxAndTy bdirs gdirs args) => do
        ...
        let ⟨args,id_gen_goal,propaGs,nGs,goalSpawn⟩ ← args.foldlM (fun ⟨args,id_gen_goal,propaGs,nGs,goalSpawn⟩ bt => do
          ...
          mtrace on .zero with s!"[propagateForBackAssembly] propaGs : {← propaGs.foldlM ListProd.nil (fun x y z => return .cons x (← ppExpr y) z)}"
          mtrace on .zero with s!"[propagateForBackAssembly] nnGs : {nnGs}"
          return ⟨a :: args, id_gen_goal, propaGs, .union nGs nnGs, goalSpawn⟩)
          (⟨[],id_gen_goal,propaGs, .empty, goalSpawn⟩ : Prod5 (List BackTree) Nat (ListProd Nat Expr) (UInt32Array) (Array (OptionProd Nat Nat)))
        return ⟨.ofIntro pass back_id gnIdxAndTy bdirs (.union gdirs nGs) args, id_gen_goal, propaGs, nGs, goalSpawn⟩
  do
...
partial def BackTree.addUnis (pass : Nat) (bt : BackTree)
  (unif_id : UInt32Array) (ta : ListProd3 Nat Nat Expr)
  : BackTree :=
  trace set TracingFlags.none in
  -- trace on .one with s!"[BackTree.addUnis] call on pass {pass} unif_id {unif_id} bt {← bt.pp 0}" in
  let affectedBids := ta.foldl UInt32Array.empty (fun x _ _ R => R.oInsert x.toUInt32)
  ...
  let rec go : BackTree → BackTree
    ...
    | x@(.ofGoal _ j t bdirs gdirs ts) =>
        if (bdirs.inter affectedBids).isEmpty
        then x
    ...
    | x@(.ofBack _ i md n bdirs gdirs ts) => Id.run do
        ...
        for j in [:ts.size] do
          ...
          | .none =>
              if !((bdirs[j]!.inter affectedBids).isEmpty)
              then
                affected := true
                ts := ts.modify j go
          | .some _ _ val =>
