...
theorem testProof (l L : List Nat) : l.dedup.dedup.length + L.length ≤ (l ++ L).length:= by
  rw [dedup_idem, length_append] -- ⊢ l.dedup.length + L.length ≤ l.length + L.length
  apply Nat.add_le_add_right -- ⊢ l.dedup.length ≤ l.length
  apply Sublist.length_le -- ⊢ l.dedup <+ l
  apply dedup_sublist
