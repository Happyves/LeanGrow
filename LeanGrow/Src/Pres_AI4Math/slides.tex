...
\subtitle{with an introduction to low level Lean metaprogramming}
...
\begin{frame}{Interactive theorem provers}
  ...
  software for writing \textbf{machine checkable maths} in a nice way.
  \medskip
  \begin{itemize}
    \item The user supplies the ideas
    \item The machine handles the bookkeeping
          and rejects anything that does not \alert{typecheck}
    \item Established systems: \alert{Lean}, Rocq/Coq, Isabelle/HOL, Agda, \dots
  \end{itemize}
  \medskip
  \begin{block}{Lean 4}
    A proof assistant \emph{and} a full programming language, built on
    \alert{dependent type theory}.
    \begin{itemize}
      \item Lean is largely implemented in Lean, with first-class
            \alert{metaprogramming} --- this is how tactics like \grow{} are written
      \item Backed by a large mathematical library, \emph{Mathlib}, and soon \emph{CSlib}
    \end{itemize}
  \end{block}
\end{frame}
...
\end{lstlisting}
  \begin{itemize}
    ...
    \item Lean has nice machinery to translate litterals to this representation
  \end{itemize}
\end{frame}

%--- Propositions and predicates --------------------------------------
\begin{frame}[fragile]{Propositions are types: the $\leq$ relation}
\begin{lstlisting}
...
\end{lstlisting}
  \begin{itemize}
    \item \texttt{Prop} is the type of \alert{propositions}. \texttt{Nat.leq n m}
          is the proposition ``$n \leq m$''
...
\begin{frame}[fragile]{Proofs are terms \hfill (1/3)}
...
\begin{frame}[fragile]{Proofs are terms \hfill (2/3)}
...
\end{lstlisting}
  \begin{itemize}
    \item Applying \texttt{leq\_step} \alert{reduces} the goal:
          it now suffices to prove $0 \leq 1$
          \\In proof search, this is a \alert{backward step}
    \item The three arguments fix $n := 0$, $m := 1$, and the last argument,
          \\the next goal, has a type the \textit{depends} on the previous arguments
  \end{itemize}
\end{frame}

\begin{frame}[fragile]{Proofs are terms \hfill (3/3)}
...
\end{lstlisting}
  \begin{itemize}
    ...
    \item Had we written \texttt{Nat.leq\_zero Nat.two} instead, Lean would have thrown an error
  \end{itemize}
\end{frame}
...
\end{lstlisting}
  \begin{itemize}
    \item \texttt{\{T : Sort u\}} is an \alert{implicit} argument (in braces):
          Lean infers it.
    \item \texttt{Eq.rewrite} is the \alert{substitution principle}: equals may
          replace equals inside any \texttt{expr}.
  \end{itemize}
\end{frame}
...
\end{lstlisting}
  \begin{itemize}
    \item Named arguments \texttt{(expr := \ldots)} pin down the implicit
          predicate.
    \item \texttt{Eq.rewrite} then delivers \texttt{expr b}, which is $b = a$.
  \end{itemize}
\end{frame}
...
\end{lstlisting}
  \begin{itemize}
    \item One principle, two readings, depending on where \texttt{expr} lands:
      \begin{itemize}
      ...
      \end{itemize}
    \item Give a \texttt{base} case and a \texttt{step}, and get a result at any
          \texttt{n}.
  \end{itemize}
\end{frame}
...
\begin{frame}{Backward search: many roads out of a goal}
  ...
  \begin{itemize}
    \item[\textcolor{sortcolor}{$\blacktriangleright$}]
      ...
      \\{\footnotesize This is the road our earlier proof took.}
...
\begin{frame}{The search space is an AND--OR tree}
  ...
  \begin{itemize}
    \item \alert{OR node} (a \emph{goal}): proven if \emph{some} applicable step
          closes it.
    \item \alert{AND node} (a \emph{step}): succeeds only if \emph{all} the
          subgoals it produces are proven
  \end{itemize}
\end{frame}

%--- grow / Aesop / sandbox -------------------------------------------
\begin{frame}[fragile]{Searching this tree in practice}
  \begin{itemize}
    \item \grow{} explores this AND--OR tree automatically, in the spirit of
          \alert{Aesop}
    \item To keep the search finite and legible, we restrict it to a small,
          fixed set of lemmas --- a \alert{sandbox}.
  \end{itemize}
\begin{lstlisting}
def stdSanbox := #[`List.dedup_cons_of_mem', `List.length_append,
  ...
  -- the only lemmata the search is allowed to use

grow_load_sandbox stdSanbox ; elimSandbox ; funIndSandbox
\end{lstlisting}
  Our running examples live in the world of \alert{lists}: \texttt{length},
  \texttt{dedup} (removing duplicates), sublists / prefixes (\texttt{<+}),
  and \texttt{Subset}.
\end{frame}
...
theorem test (l L : List Nat) (h : (l ++ L).length = 42) :
  ...
  grows
\end{lstlisting}
  \grow{} finds a term \alert{close to} what a user would write
  (\texttt{rw [$\leftarrow$ length\_append]; exact h}):
\begin{lstlisting}
...
%--- Example: test' ---------------------------------------------------
\begin{frame}[fragile]{Example 2: a proof, but a roundabout one}
\begin{lstlisting}
...
\item The whole point of Lean is that a proof is a \alert{term the kernel
      checks}. A search that cut corners here would produce proofs we
      \alert{cannot trust}.
\item So every step the search takes must assemble into a genuine,
      type-correct term.
\end{itemize}
...
\begin{itemize}
	\item \texttt{rw} matches on the goal's surface term and refuses to descend
	under a binder.
	\item \texttt{simp\_rw} is built to rewrite \alert{under binders} --- so here
	it goes through.
\end{itemize}
...
\end{lstlisting}
  \begin{itemize}
    \item The lemma carries a \alert{side condition} $0 < b$. Under the binder
          the hypothesis \texttt{h : 1 < b+1} that would discharge it is in
          scope, yet \alert{both tactics give up}.
...
\end{lstlisting}
  \begin{itemize}
    \item Here \texttt{n+m} appears inside the \alert{dependent type}
          \texttt{Fin (n+m+1)} --- the very type of \texttt{x}.
    \item Rewriting it to \texttt{m+n} would retype \texttt{x}, so the motive is
          not type-correct and both tactics won't work.
  \end{itemize}
\end{frame}
...
\begin{frame}{Set-tries: taking forward steps efficiently}
  ...
  \begin{block}{Abstractly}
    A family of \alert{target sets} (each theorem's hypotheses), a \alert{query
    set} (our current facts): report every target set that is
    $\subseteq$ the query set.
  \end{block}
  \medskip
  A \alert{set-trie} stores the family so that a \alert{common subset} of several
  targets is tested \alert{once}, not once per theorem.
\end{frame}
...
\begin{frame}{Sharing common hypotheses}
  ...
  \begin{tikzpicture}[
    ...
    \draw[ed] (perm)--(idem); \draw[ed] (perm)--(pdd);
  \end{tikzpicture}}
  \end{center}
  A root-to-leaf path lists the hypotheses a theorem needs; a shared prefix is stored \alert{once}.
\end{frame}
...
\begin{frame}{Building a set-trie: a greedy heuristic}
  Family $\{\{a,b,x\},\{a,c,x\},\{c,d\},\{c,e,y\},\{e,f,y\}\}$. Goal: a trie whose
  total key size (summed over nodes) is small.
  \begin{center}
  ...
  \end{tikzpicture}}
  \end{center}
  Pick the element most common among siblings, make it a shared parent; stop when
...
\begin{frame}{Path indexing: storing sets of expressions}
  \begin{itemize}
    \item Like \alert{discrimination trees}, a path index stores a set of
          syntax trees (\texttt{Expr}) so we can query them efficiently.
    ...
    \item Allowed for \alert{scoring heuristic} of the next section.
  \end{itemize}
\end{frame}
...
\end{center}
  ...
  \begin{itemize}
	...
	\\Ex.: if $n$ is a metavariable, match $(a*b)+m=45$ with assignment
...
\begin{frame}{A memorisation heuristic for steps}
  ...
  \begin{itemize}
    ...
    \item \alert{Memorisation.} A theorem is a promising candidate when, in
      ...
      \begin{itemize}
        \item the current goal \emph{matches} a recorded goal, and
        \item the context \emph{contains} the recorded hypotheses.
      \end{itemize}
    \item We won't compare goals syntactically: the recorded patterns get
          \alert{generalised} with a path-index procedure --- \emph{covered later}.
  \end{itemize}
  \medskip
...
theorem testProof (l L : List Nat) :
  ...
  apply dedup_sublist
\end{lstlisting}
  A sample records the \alert{lifted context}, the \alert{goal}, the
...
\\For each theorem, we merge all its recorded goals into \alert{one path index}, then \alert{generalise} it:
  ...
  \alert{unifies} with this pattern --- \emph{without} assigning the goal's own
  variables.
\end{frame}
...
\begin{frame}{Generalising the index}
  ...
  metavariable $f? : \mathbb{N}\to\mathbb{N}\to\mathbb{N}$. $n/m$ will now match !
\end{frame}
...
\begin{frame}[fragile]{Generalisation: open aspects}
  \begin{itemize}
    \item \alert{Metavariable garbage collection}: Generalisation can create
          ...
          referenced by the index --- these must be collected.
    \item \alert{Metavariable reuse}: We want as few metavariables as possible;
...
\begin{itemize}
	...
	\item Currently a statistical/phenomenological approach: take theorem as search step if goal and context match previously seen applications.
