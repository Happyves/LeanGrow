...
def sampleClass_back_cvbThm_core
  ...
  (withHyps : Bool)
  (init : ProcessedSamples IdxCollType)
  : MetaM (ProcessedSamples IdxCollType) := do
  mtracing
  let env ← getEnv
  ...
  for thmName in thmNames do
    ...
    samplesB := res
    let .mk res l1 l2 ← sampleCoreForw preS l1 l2 withHyps depthDig depthStart depthStop deltaFuzz zetaFuzz (.cons (.raw p) (fvs.map Expr.fvarId!).toList .nil) samplesF
    samplesB := res
    L1 := l1
    L2 := l2
...
def LeanGrow.sampleNameOfModuleName (mod : Name) :=
  "sampleScore_" ++ mod.toUnderscoreString
...
unsafe def sampleClass_back_cvbThm_ofModule
  ...
  WithImportModules (modules.map (fun x => {module := x})) {} <| fun env => do
    stdMetaRun env do
      ...
      let sampleName := .str .anonymous <| LeanGrow.sampleNameOfModuleName mod
      let res ← sampleClass_back_cvbThm_core thmNames conjTree
        depthDig depthStart depthStop deltaFuzz zetaFuzz empty
        singleton insert intersect empty? false
        (.mk sampleName .empty .empty (.mk 0 .dead #[]) .empty 1 #[])
      let path := FilePath.join cachePath ((FilePath.toString (LeanGrow.sampleNameOfModuleName mod)))
      pickle path res
      for reg in regs do
        reg.free
