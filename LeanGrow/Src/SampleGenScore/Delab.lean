...
def Lean.Expr.consumeId (h : Expr) (as : Array Expr) : Option Expr :=
  match h with
  | .const n _ =>
    if n == ``id
    then
...
partial def delabSample
  ...
  | .app .. =>
    ...
    match appH.consumeId appA with
    | .some nx => delabSample preProcessed nx l1 l2
    | _ =>
...
partial def delabDig
  ...
  | .app .. =>
    ...
    let appA := e.getAppArgs
    match appH.consumeId appA with
    | .some nx => delabDig preProcessed nx l1 l2
    | _ =>
...
partial def delabTopBack
  ...
  | .app .. =>
    ...
    let appA := e.getAppArgs
    match appH.consumeId appA with
    | .some nx => delabTopBack preProcessed conjable nx l1 l2 havelift
    | _ =>
...
partial def delabTopForw
  ...
  | .app .. =>
    ...
    let appA := e.getAppArgs
    match appH.consumeId appA with
    | .some nx => delabTopForw preProcessed nx l1 l2
    | _ =>
...
partial def delabTopForw_withHyps
  (preProcessed : CTrie SimpCongrTheorem)
  (e : Expr) (l1 : LocalContext) (l2 : LocalInstances)
  : MetaM (Prod4 SampleData (List Expr) LocalContext LocalInstances) := do
  mtracing
  ...
  | .app .. =>
    ...
    let appA := e.getAppArgs
    match appH.consumeId appA with
    | .some nx => delabTopForw_withHyps preProcessed nx l1 l2
    | _ =>
